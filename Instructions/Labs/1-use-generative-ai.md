...
1. Open [Bing.com](https://www.bing.com?azure-portal=true) and sign in with your personal account.
...
1. Type in a prompt: `Where are 3 places I can go to find fewer crowds?`. Notice that while Bing Copilot is able to give a related response, it can drop earlier "memories" of the conversation thread as it continues. As a result, the responses you get may not be directly related to traveling in the winter. This is largely to do with token input limitations. Recall from the module that language modeling breaks up sentences into tokens, or bits of words. When chat "remembers" earlier parts of a conversation, it is because it has saved a certain amount of tokens from the conversation. As new tokens are introduced via your new prompts and responses, chat will let go of older tokens. 
...
1. In the response, there is text at the bottom that reads "Powered by DALL-E". Click on that text to see **Image creator (preview)**. Consider how DALL-E is based on large language models as your natural language input generates images. 
...
1. Finally, type in a prompt: `What are 3 examples of generative AI helping people?`. In this example, although you did not clear the conversation by clicking **New Topic**, you are able to generate responses related to your current prompt. You will find you do not always need to clear the topic to ask about a new one.  

What you have tested out with Bing Copilot is one example of what can be built with Azure OpenAI Service. 